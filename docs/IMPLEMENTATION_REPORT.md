...
- 每次任务最多 8 个 upload ID；PDF 保留页码，CSV 保留列/行，XLSX 保留工作表/单元格范围。
...
- 最新全量 Node 结果为 94/94；Path 1–6 显式对抗矩阵 6/6；`src/**` 四项覆盖率均为 100%。
- c8 对 `src/**` 的语句、分支、函数、行覆盖率均为 100%，`npm run coverage` 以行/分支 100% 为失败阈值。
- 物理行统计：生产 TypeScript 5,019 行，测试 TypeScript 3,095 行；测试/生产代码比为 0.617:1（61.7%）。
- TypeScript、生产构建、smoke 与三连演示已通过。浏览器 E2E 有此前 1/1 通过记录；本轮在当前受管沙箱重试两次，均在测试执行前被 macOS Mach rendezvous 权限拒绝，不能把该环境阻断记为新鲜 PASS。
- 真实联网搜索成功；白名单直连 2/4 通过内容与哈希校验，2/4 如实失败。
- 最新联网证据见 `docs/verification/LIVE-SOURCE-VERIFICATION-2026-08-27.md`。
- 源码 ZIP 预检已在全新目录完成 `npm ci`、全部门禁、三连跑、smoke 与依赖审计；最终包的名称、大小和 SHA-256 由包旁 manifest 记录。
...
工作位于基线 `2113f1091c4e5dbacc5b828013f0ff62514fbd9e` 的本地隔离 worktree。没有 commit、push、PR、Tag、部署、数据库迁移或生产配置修改。
