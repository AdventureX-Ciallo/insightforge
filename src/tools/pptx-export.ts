...
const EAST_ASIA_FONT = "Arial Unicode MS";
...
class SlideBuilder {
  ...
  text(value: string, x: number, y: number, w: number, h: number, options: TextOptions) {
    const id = this.id++;
    const size = Math.round(options.size * 100);
    ...
    const paragraphs = String(value).split("\n").map((line) => `<a:p><a:pPr algn="${align}"/><a:r><a:rPr lang="zh-CN" sz="${size}"${options.bold ? ' b="1"' : ""}><a:solidFill><a:srgbClr val="${color}"/></a:solidFill><a:latin typeface="${font}"/><a:ea typeface="${EAST_ASIA_FONT}"/></a:rPr><a:t>${escapeXml(line)}</a:t></a:r><a:endParaRPr lang="zh-CN" sz="${size}"/></a:p>`).join("");
